...
const validController_1 = __importDefault(require("./controller/validController"));
const getData_1 = __importDefault(require("./controller/getData"));
const manageData_1 = __importDefault(require("./controller/manageData"));
...
app.get("/", async (req, res) => {
    const { getDataMeeting } = (0, getData_1.default)();
    try {
...
app.get("/book/:id", async (req, res) => {
    ...
    const { getDataMeetingById } = (0, getData_1.default)();
    try {
...
app.get("/book/:id/booking", async (req, res) => {
    const id = req.params.id;
    const { getDataBooking } = (0, getData_1.default)();
    try {
...
app.put("/book/:status/:id/status", async (req, res) => {
    const id = req.params.id;
    const status = req.params.status;
    const payload = req.body.payload;
    const { updateDataStatus } = (0, manageData_1.default)();
    const { getDataMeetingById, getDataBooking } = (0, getData_1.default)();
    const { checkValidMeeting, checkValidDuplication, message } = (0, validController_1.default)();
    try {
        if (!id || !status)
        ...
        if (status === "booked") {
            ...
            const resultValidMeeting = checkValidMeeting(result.data[0], payload.start_time_utc, payload.end_time_utc, payload.user_timezone);
            ...
            const isDuplicate = checkValidDuplication(resultBooking.data, payload.start_time_utc, payload.end_time_utc);
            if (!isDuplicate)
...
app.post("/book", async (req, res) => {
    ...
    const { checkValid, checkValidMeeting, checkValidDuplication, message } = (0, validController_1.default)();
    const { getDataMeetingById, getDataBooking } = (0, getData_1.default)();
    const { insertData, resultInsert } = (0, manageData_1.default)();
    ...
    try {
        ...
        const validMeeting = checkValidMeeting(organizerData, start_time_utc, end_time_utc, user_timezone);
        if (!validMeeting)
            return res.status(200).json({ error: message.error });
        const resultBooking = await getDataBooking(organizer_id, "");
        if (resultBooking.error)
            return res.status(400).json({ error: resultBooking.error });
        const validDuplication = checkValidDuplication(resultBooking.data, start_time_utc, end_time_utc);
        if (!validDuplication)
            return res.status(200).json({ error: message.error });
        ...
        if (resultInsert.error)
            return res.status(200).json({ error: resultInsert.error });
...
app.put("/settings/:edit/:id", async (req, res) => {
    ...
    const { updateData, updateDataWorkingHours, updateDataBlackouts } = (0, manageData_1.default)();
    const { getDataMeetingById, getDataBooking } = (0, getData_1.default)();
    const { checkValidSettingWorkingHours, checkValidSettingBlackouts, checkValidSettingGeneral, message } = (0, validController_1.default)();
        ...
        switch (req.params.edit) {
            case "general":
                ...
                if (resultUpdate.error)
                    return res.status(200).json({ error: resultUpdate.error });
            ...
            case "working_hours":
                ...
                if (!validSettingWorkingHours)
                    return res.status(200).json({ error: message.error });
                const resultUpdateWorkingHours = await updateDataWorkingHours(numberId, working_hours);
                if (resultUpdateWorkingHours.error)
                    return res.status(200).json({ error: resultUpdateWorkingHours.error });
            ...
            case "blackouts":
                ...
                if (!validSettingBlackouts)
                    return res.status(200).json({ error: message.error });
                const resultUpdateBlackouts = await updateDataBlackouts(numberId, blackouts);
                if (resultUpdateBlackouts.error)
                    return res.status(200).json({ error: resultUpdateBlackouts.error });
